...
class Qmess_saimatkhau:
    ...
    def show_window(parent=None):
        ...
        try:
            # Load hình ảnh
            dn_Qmess_saimk_image = PhotoImage(file=relative_to_assets("Qmess_nho.png"))
            dn_Qmess_saimk_thulai_image = PhotoImage(file=relative_to_assets("Qmess_thulai.png"))
        except Exception:
            return

        # Hiển thị ảnh nền
        canvas.create_image(225.0, 106.0, image=dn_Qmess_saimk_image)
        ...
        # Nút thử lại
        dn_Qmess_saimk_thulai = Button(
            window,
            image=dn_Qmess_saimk_thulai_image,
            borderwidth=0,
            highlightthickness=0,
            command=close_window,
            relief="flat"
        )
        dn_Qmess_saimk_thulai.place(x=151.0, y=137.0, width=148.0, height=34.0)
