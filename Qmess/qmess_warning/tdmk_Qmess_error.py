...
class Qmess_tdmk_error:
    ...
    def show_window(parent=None, on_close=None):  # Thêm on_close vào tham số
        ...
        try:
            Qmess_tdmk_error_image = PhotoImage(file=relative_to_assets("Qmess_nho.png"))
            Qmess_tdmk_error_thulai_image = PhotoImage(file=relative_to_assets("Qmess_thulai.png"))
        except Exception:
            return

        # Hiển thị ảnh nền
        canvas.create_image(225.0, 106.0, image=Qmess_tdmk_error_image)

        # Hiển thị thông báo đăng ký thành công
        canvas.create_text(
            225, 100,
            text="      Lỗi xảy ra trong\nquá trình thay đổi mật khẩu",
            fill="#000000",
        ...
        # Nút OK
        Qmess_tdmk_error_thulai = Button(
            window,
            image=Qmess_tdmk_error_thulai_image,
            borderwidth=0,
            highlightthickness=0,
            command=close_window,  # Đóng cửa sổ khi bấm nút
            relief="flat"
        )
        Qmess_tdmk_error_thulai.place(x=151.0, y=137.0, width=148.0, height=34.0)
