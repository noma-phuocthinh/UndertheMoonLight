...
class Qmess_gender_kb:
    ...
    def show_window(parent=None, on_close=None):
        ...
        try:
            kb_Qmess_gender_image = PhotoImage(file=relative_to_assets("Qmess_nho.png"))
            kb_Qmess_gender_thulai_image = PhotoImage(file=relative_to_assets("Qmess_thulai.png"))
        except Exception:
            window.destroy()
            return

        # Hiển thị ảnh nền
        canvas.create_image(225.0, 106.0, image=kb_Qmess_gender_image)
        ...
        close_button = Button(
            window,
            image=kb_Qmess_gender_thulai_image,
            borderwidth=0,
            highlightthickness=0,
